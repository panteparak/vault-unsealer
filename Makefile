...
# autounseal.vault.io/vault-unsealer-bundle:$VERSION and autounseal.vault.io/vault-unsealer-catalog:$VERSION.
IMAGE_TAG_BASE ?= autounseal.vault.io/vault-unsealer
...
KIND_CLUSTER ?= vault-unsealer-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name vault-unsealer-builder
	$(CONTAINER_TOOL) buildx use vault-unsealer-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm vault-unsealer-builder
	rm Dockerfile.cross
...
.PHONY: cross-compile
cross-compile: manifests generate fmt vet ## Build manager binary for multiple platforms.
	@mkdir -p bin
